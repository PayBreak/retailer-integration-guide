...
### Simple Loan Request: checkout_type 1
...
For readability the merchant_hash have been truncated. Let’s explain those fields in more detail:
...
merchant_hash | string | The merchant hash is generated by concatenating the following fields: `additional_data` (if present) + `checkout_type` + `checkout_version` + `merchant_installation` + `order_amount` + `order_description` + `order_extendable` + `order_reference` + `order_validity`. The concatenated string should then be used with the Shared Secret Key to create a HMAC-SHA256 message digest which should be represented as lowercase hexadecimal digits. This is identical to the method described in API Security.

> All fields except additional_data are required.

### Extended Loan Request: checkout_type 2
...
<form method="post" action="https://checkout-test.paybreak.com/">
    ...
    <input type="hidden" name="chceckout_version" value="3.1" />
...
additional_data | string | A base64 encoded JSON-formatted string containing additional data as described in sections below. This is an optional field and if present must be added to the request hash.
merchant_hash | string | The merchant hash is generated by concatenating the following fields: additional_data (if present) + checkout_type + checkout_version +  merchant_installation + order_amount + order_description + order_extendable + order_items + order_reference + order_validity. The concatenated string should then be used with the Shared Secret Key to create a HMAC-SHA256 message digest which should be represented as lowercase hexadecimal digits. This is identical to the method described in API Security.

> All fields except additional_data are required.

#### Order Items

The order_items field must contain a base64 encoded JSON-formatted string
containing an ordered list of items that constitute the order. For the above
order the JSON could be represented as shown:
...
alternative_fulfilment | bool | Flag saying if the fulfilment address is different to the customer's current address. To use this field the Alternative Delivery Address Feature must be enabled for the installation and if it's enabled this field is required.
address_fulfilment | string | A JSON formatted string containing an Address object. To use this field the Alternative Delivery Address Feature must be enabled for this installation. This fields represents the customer's fulfilment address. Where goods are delivered, this is the address they must be delivered to.
...
If you want add any of these features to your integration contact our Merchant
Support Team.
...
> If feature is enabled, field address_current and alternative_fulfilment are
> required in all Loan Requests.
